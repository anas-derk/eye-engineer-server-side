...
async function sendApproveOfficeEmail(email, password, adminId, officeId, language) {
    const result = await getPasswordForBussinessEmail(process.env.BUSSINESS_EMAIL);
    if (!result.error) {
        const templateContent = readFileSync(join(__dirname, "..", "..", "assets", "email_templates", "accept_add_office_request.ejs"), "utf-8");
        const compiledTemplate = compile(templateContent);
        ...
        return new Promise((resolve, reject) => {
            emailsUtils.getTransporter(result.data).sendMail({
                from: `${process.env.WEBSITE_NAME} <${process.env.BUSSINESS_EMAIL}>`,
                to: email,
                subject: `الموافقة على طلب إضافة مكتب هندسي جديد لدى ${process.env.WEBSITE_NAME}`,
                html: htmlContentAfterCompilingEjsTemplateFile,
            }, function (error, info) {
                if (error) reject(error);
                resolve({
                    msg: "Sending Approve Email On Office Process Has Been Successfully !!",
                    error: false,
                    data: {},
                });
            });
...
async function sendRejectOfficeEmail(email, language) {
    const result = await getPasswordForBussinessEmail(process.env.BUSSINESS_EMAIL);
    if (!result.error) {
        const templateContent = readFileSync(join(__dirname, "..", "..", "assets", "email_templates", "reject_add_office_request.ejs"), "utf-8");
        const compiledTemplate = compile(templateContent);
        const htmlContentAfterCompilingEjsTemplateFile = compiledTemplate({ language });
        return new Promise((resolve, reject) => {
            emailsUtils.getTransporter(result.data).sendMail({
                from: `${process.env.WEBSITE_NAME} <${process.env.BUSSINESS_EMAIL}>`,
                to: email,
                subject: `رفض طلب إضافة مكتب هندسي جديد لدى ${process.env.WEBSITE_NAME}`,
                html: htmlContentAfterCompilingEjsTemplateFile,
            }, function (error, info) {
                if (error) reject(error);
                resolve({
                    msg: "Sending Reject Email On Office Process Has Been Successfully !!",
                    error: false,
                    data: {},
                });
            });
...
async function sendConfirmRequestAddOfficeArrivedEmail(email, language) {
    const result = await getPasswordForBussinessEmail(process.env.BUSSINESS_EMAIL);
    if (!result.error) {
        const templateContent = readFileSync(join(__dirname, "..", "..", "assets", "email_templates", "confirm_request_add_office_arrived.ejs"), "utf-8");
        const compiledTemplate = compile(templateContent);
        const htmlContentAfterCompilingEjsTemplateFile = compiledTemplate({ language });
        return new Promise((resolve, reject) => {
            emailsUtils.getTransporter(result.data).sendMail({
                from: `${process.env.WEBSITE_NAME} <${process.env.BUSSINESS_EMAIL}>`,
                to: email,
                subject: `تأكيد طلب إضافة مكتب هندسي جديد لدى ${process.env.WEBSITE_NAME}`,
                html: htmlContentAfterCompilingEjsTemplateFile,
            }, function (error, info) {
                if (error) reject(error);
                resolve({
                    msg: "Sending Confirmation Of Office Addition Request At Ubuyblues Email Process Has Been Successfully !!",
                    error: false,
                    data: {},
                });
            });
...
async function sendBlockOfficeEmail(email, adminId, officeId, language) {
    const result = await getPasswordForBussinessEmail(process.env.BUSSINESS_EMAIL);
    if (!result.error) {
        const templateContent = readFileSync(join(__dirname, "..", "..", "assets", "email_templates", "block_office.ejs"), "utf-8");
        const compiledTemplate = compile(templateContent);
        ...
        return new Promise((resolve, reject) => {
            emailsUtils.getTransporter(result.data).sendMail({
                from: `${process.env.WEBSITE_NAME} <${process.env.BUSSINESS_EMAIL}>`,
                to: email,
                subject: `حظر مكتب هندسي في ${process.env.WEBSITE_NAME}`,
                html: htmlContentAfterCompilingEjsTemplateFile,
            }, function (error, info) {
                if (error) reject(error);
                resolve({
                    msg: "Sending Block Email The Office Process Has Been Successfully !!",
                    error: false,
                    data: {},
                });
            });
...
async function sendDeleteOfficeEmail(email, adminId, officeId, language) {
    const result = await getPasswordForBussinessEmail(process.env.BUSSINESS_EMAIL);
    if (!result.error) {
        const templateContent = readFileSync(join(__dirname, "..", "..", "assets", "email_templates", "delete_office.ejs"), "utf-8");
        const compiledTemplate = compile(templateContent);
        const htmlContentAfterCompilingEjsTemplateFile = compiledTemplate({ adminId, officeId, language });
        return new Promise((resolve, reject) => {
            emailsUtils.getTransporter(result.data).sendMail({
                from: `${process.env.WEBSITE_NAME} <${process.env.BUSSINESS_EMAIL}>`,
                to: email,
                subject: `حذف مكتب هندسي في ${process.env.WEBSITE_NAME}`,
                html: htmlContentAfterCompilingEjsTemplateFile,
            }, function (error, info) {
                if (error) reject(error);
                resolve({
                    msg: "Sending Delete Email The Office Process Has Been Successfully !!",
                    error: false,
                    data: {},
                });
            });
    ...
    if (!result.error) {
        ...
        return new Promise((resolve, reject) => {
            emailsUtils.getTransporter(result.data).sendMail({
                from: `${process.env.WEBSITE_NAME} <${process.env.BUSSINESS_EMAIL}>`,
                to: email,
                subject: `استقبال طلب إضافة مكتب هندسي في ${process.env.WEBSITE_NAME}`,
                html: htmlContentAfterCompilingEjsTemplateFile,
            }, function (error, info) {
                if (error) reject(error);
                resolve({
                    msg: "Sending Receive Add Office Request Email To Website Owner Process Has Been Successfully !!",
                    error: false,
                    data: {},
                });
            });
